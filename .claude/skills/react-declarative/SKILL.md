...
description: React の宣言的設計パターン。コンポーネントは「何をするか」を宣言し、「どうやるか」はカスタムフック・utils に委譲する。コンポーネント実装時やコードレビュー時に使用。component-common(構成・命名)や zustand-pattern(状態管理)と組み合わせて使う。
---
